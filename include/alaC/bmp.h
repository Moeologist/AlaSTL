...
int write_bmp(const void *buffer, int width, int height, const char *filename, int withalpha) {
	...
	if (withalpha)
		channels = 4;
	BITMAPFILEHEADER bf = {0x4d42, 14 + 40 + width * height * channels, 0, 0, 14 + 40};
	BITMAPINFOHEADER bi = {40, width, height, 1, (unsigned short)(channels * 8), 0, width * height * channels, 72, 72, 0, 0};
	...
	for (int i = height - 1; i >= 0; i--) {
		fwrite((unsigned char *)buffer + i * width * channels, 1, width * channels, f);
...
int read_bmp(const void *buffer, int *width, int *height, const char *filename, int *withalpha) {
	...
	*width = bi.biWidth, *height = bi.biHeight;
	for (int i = *height - 1; i >= 0; i--) {
		fread((unsigned char *)buffer + i * *width * channels, 1, *width * channels, f);
		int fill = *width * channels % 4;
		if (fill) {
			unsigned char fillzero[4] = {0};
