 ...
 */
inline std::string encode(const std::string &binaryText) {
    constexpr char ALPHABET[65] =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

    std::string encoded((binaryText.size() / 3 + (binaryText.size() % 3 > 0)) << 2,
                        PADDING_CHAR);
 ...
 */
inline std::string decode(const std::string &base64Text) {
    constexpr uint8_t DECODED_ALPHBET[128] = {
    ...
    assert((base64Text.size() & 3) == 0 &&
           "The base64 text to be decoded must have a length devisible by 4!");
    ...
    for (uint32_t i = (base64Text.size() >> 2) - (numPadding != 0); i;
        ...
        temp = DECODED_ALPHBET[bytes[0]] << 18 | DECODED_ALPHBET[bytes[1]] << 12 |
               DECODED_ALPHBET[bytes[2]] << 6 | DECODED_ALPHBET[bytes[3]];
    ...
    switch (numPadding) {
        case 2:
            temp = DECODED_ALPHBET[bytes[0]] << 18 | DECODED_ALPHBET[bytes[1]]
                                                         << 12;
            currDecoding[0] = tempBytes[2];
            break;

        case 1:
            temp = DECODED_ALPHBET[bytes[0]] << 18 |
                   DECODED_ALPHBET[bytes[1]] << 12 |
                   DECODED_ALPHBET[bytes[2]] << 6;
            currDecoding[0] = tempBytes[2];
            currDecoding[1] = tempBytes[1];
            break;
    }
