...
template<class T>
concept __boolean_testable = __boolean_testable_impl<T> && requires(T &&t) {
...
template<class T>
concept __integeral_arithmetic = integral<T> && !same_as<T, bool>;
...
struct _cpo_fn {
    ...
    requires(!__enable_adl<T &, T &> && move_constructible<T> &&
             assignable_from<T &, T>) constexpr void
    ...
    constexpr void operator()(T (&t)[N], U (&u)[N]) const
        noexcept(noexcept((*this)(t[0], u[0]))) requires requires {
        (*this)(t[0], u[0]);
    }
    {
...
&, const remove_reference_t<U> & >
       &&totally_ordered<common_reference_t<const remove_reference_t<T> &,
                                            const remove_reference_t<U> &>> &&
           equality_comparable_with<T, U> &&requires(
               const remove_reference_t<T> &t, const remove_reference_t<U> &u) {
    { t < u } -> __boolean_testable;
    { t > u } -> __boolean_testable;
    { t <= u } -> __boolean_testable;
    { t >= u } -> __boolean_testable;
