...
ALA_CONCEPT derived_from =
    is_base_of_v<Base, Derived> &&
    is_convertible_v<const volatile Derived *, const volatile Base *>;

struct _convertible_to_c {
    template<typename From, typename To>
    auto _requires() -> decltype(static_cast<To>(declval<From>()));
};

template<typename From, typename To>
ALA_CONCEPT convertible_to = is_convertible_v<From, To> &&
                             _requires<_convertible_to_c, From, To>;

template<class T>
ALA_CONCEPT __boolean_testable_impl = convertible_to<T, bool>;

struct __boolean_testable_c {
    template<typename T>
    auto _requires(T &&t)
...
template<class T>
ALA_CONCEPT __boolean_testable = __boolean_testable_impl<T> &&
                                 _requires<__boolean_testable_c, T>;
...
struct _common_with_c {
    template<typename T, typename U>
    auto _requires() -> decltype(static_cast<common_type_t<T, U>>(declval<T>()),
                                 static_cast<common_type_t<T, U>>(declval<U>()));
};

template<typename T, typename U>
ALA_CONCEPT common_with =
    same_as<common_type_t<T, U>, common_type_t<U, T>> &&
    _requires<_common_with_c, T, U> &&
...
template<class T>
ALA_CONCEPT __integeral_arithmetic = integral<T> && !same_as<T, bool>;

template<typename T>
ALA_CONCEPT floating_point = is_floating_point_v<T>;

struct _assignable_from_c {
...
template<class LHS, class RHS>
ALA_CONCEPT assignable_from =
    is_lvalue_reference_v<LHS> &&
    common_reference_with<const remove_reference_t<LHS> &,
                          const remove_reference_t<RHS> &> &&
    _requires<_assignable_from_c, LHS, RHS>;

template<class T>
ALA_CONCEPT destructible = is_nothrow_destructible_v<T>;

template<class T, class... Args>
ALA_CONCEPT constructible_from = destructible<T> &&
                                 is_constructible_v<T, Args...>;

struct _default_initializable_c {
    template<class T>
    auto _requires() -> decltype(T{}, ::new(static_cast<void *>(nullptr)) T);
};

template<class T>
ALA_CONCEPT default_initializable = constructible_from<T> &&
                                    _requires<_default_initializable_c, T>;

template<class T>
ALA_CONCEPT move_constructible = constructible_from<T, T> && convertible_to<T, T>;

struct _copy_constructible_c {
    template<class T>
    auto _requires()
        -> enable_if_t<move_constructible<T> && constructible_from<T, T &> &&
...
template<class T>
ALA_CONCEPT copy_constructible = _requires<_copy_constructible_c, T>;
...
template<class T>
ALA_CONCEPT swappable = _requires<_swappable_c, T>;

struct _swappable_with_c {
    template<class T, class U>
    auto _requires(T &t, U &u)
...
template<class T, class U>
ALA_CONCEPT swappable_with = common_reference_with<T, U> &&
                             _requires<_swappable_with_c, T, U>;

struct __weakly_equality_comparable_with_c {
    template<class T, class U>
...
ALA_CONCEPT __weakly_equality_comparable_with =
    _requires<__weakly_equality_comparable_with_c, T, U>;
...
template<class T>
ALA_CONCEPT totally_ordered = equality_comparable<T> &&
                              _requires<_totally_ordered_c, T>;

struct _totally_ordered_with_c {
    template<class T, class U>
    auto _requires(const remove_reference_t<T> &t, const remove_reference_t<U> &u)
        -> enable_if_t<__boolean_testable<decltype(t < u)> &&
                       __boolean_testable<decltype(t > u)> &&
...
template<class T, class U>
ALA_CONCEPT totally_ordered_with =
    totally_ordered<T> && totally_ordered<U> &&
    common_reference_with<const remove_reference_t<T> &,
                          const remove_reference_t<U> &> &&
    totally_ordered<common_reference_t<const remove_reference_t<T> &,
                                       const remove_reference_t<U> &>> &&
    equality_comparable_with<T, U> && _requires<_totally_ordered_with_c, T, U>;

struct _movable_c {
    template<class T>
    auto _requires() -> enable_if_t<is_object_v<T> && move_constructible<T> &&
                                    assignable_from<T &, T> && swappable<T>>;
};

template<class T>
ALA_CONCEPT movable = _requires<_movable_c, T>;

struct _copyable_c {
    template<class T>
    auto _requires() -> enable_if_t<
        copy_constructible<T> && movable<T> && assignable_from<T &, T &> &&
        assignable_from<T &, const T &> && assignable_from<T &, const T>>;
};

template<class T>
ALA_CONCEPT copyable = _requires<_copyable_c, T>;
...
template<class F, class... Args>
ALA_CONCEPT invocable = _requires<_invocable_c, F, Args...>;
