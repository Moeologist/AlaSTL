...
protected:
  ...
  struct _node_adaptor {
    ...
    ALA_NODISCARD bool empty() const noexcept { return _ptr == nullptr; }

    allocator_type get_allocator() const { return _a; }

#if IS_MAP
    key_type &key() const { return const_cast<key_type &>(_ptr->_data.first); }

    mapped_type &mapped() const { return _ptr->_data.second; }
#else
    value_type &value() const { return _ptr->_data; }
#endif
...
#if IS_UNIQ
  template <class It, class NodeType> struct _insert_return_adaptor {
    It position;
    bool inserted;
    NodeType node;
  };
...
public:
  ...
  // iterators:
  iterator begin() noexcept { return iterator(tree.begin()); }

  const_iterator begin() const noexcept { return cbegin(); }

  iterator end() noexcept { return iterator(tree.end()); }
  ...
  // observers:
  allocator_type get_allocator() const noexcept { return tree.get_allocator(); }
  ...
  // find operations:
  iterator find(const key_type &k) { return tree.template find<key_type>(k); }
...
template <class Key, class T, class Compare, class Alloc>
void swap(
    CONTAINER<Key, T, Compare, Alloc> &lhs,
...
template <class Key, class Compare, class Alloc>
void swap(
    CONTAINER<Key, Compare, Alloc> &lhs,
...
CONTAINER(It, It, Comp = Comp(), Alloc = Alloc())
    ->CONTAINER<typename iterator_traits<It>::value_type::first_type,
                typename iterator_traits<It>::value_type::second_type, Comp,
                Alloc>;
...
CONTAINER(It, It, Alloc)
    ->CONTAINER<typename iterator_traits<It>::value_type::first_type,
                typename iterator_traits<It>::value_type::second_type,
                less<typename iterator_traits<It>::value_type::first_type>,
                Alloc>;
