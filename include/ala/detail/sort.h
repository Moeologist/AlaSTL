...
void quick_sort_impl3(RanIt first, RanIt last) {
    ...
    detail::set_median(*first, *(first + length / 4), *(first + length * 2 / 4),
                       *(first + length * 3 / 4));
    const T pivot = *first;
...
void sort_impl(RanIt first, RanIt last, unsigned int depth) {
    ...
    while (last - first > 28 && depth > 0) {
        const T pivot = forward<T>(detail::median(
            forward<T>(*first), forward<T>(*(first + (last - first) / 2)),
            forward<T>(*(last - 1))));
        RanIt left = first, right = last;
        for (;; ++left) {
            while (*left < pivot)
