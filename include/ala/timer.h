...
std::string timer(Callable callable, Args... args) {
	std::string units[] = {"ns", "us", "ms", "s"};
	struct timespec before, after, info;
	clock_getinfo(CLOCK_MONOTONIC, &info);
	clock_gettime(CLOCK_MONOTONIC, &before);
	callable(args...);
	clock_gettime(CLOCK_MONOTONIC, &after);
	long long ns_time = （(after.tv_nsec - before.tv_nsec) + (after.tv_sec - before.tv_sec) * 1000000000) / info.tv_nsec;
	int unit_index = 0;
	while (ns_time > 10000 && unit_index < 4) {
		++unit_index;
		ns_time /= 1000;
