...
class vector {
  ...
  public:
	...
	_ALA_FORCEINLINE vector(value_type *m) {
		for (int i = 0; i < Size; ++i)
			_m[i] = m[i];
	}
	...
	_ALA_FORCEINLINE bool operator==(const vector<T, Size> &rhs) {
		return !(*this != rhs);
	}
	...
	// _ALA_FORCEINLINE vector<T, Size> operator+=(const vector<T, Size> &rhs) const {
	// 	for (int i = 0; i < Size; ++i)
	// 		_m[i] += rhs._m[i];
	// 	return *this;
	// }
	...
	_ALA_FORCEINLINE vector<T, Size> operator+(const T &rhs) const {
		T m[Size] = {0};
		for (int i = 0; i < Size; ++i)
			m[i] = _m[i] + rhs;
		return vector<T, Size>(m);
	...
	_ALA_FORCEINLINE const T &operator[](const int index) const {
		if (index == 0)
			return _m[0];
		if (index == 1)
			return _m[1];
	...
	_ALA_FORCEINLINE T dot(const vector<T, Size> &rhs) const {
		 T sum = 0;
		 for (int i = 0; i < Size; ++i)
		 	sum += _m[i] * rhs._m[i];
		 return sum;
...
template <class T>
_ALA_FORCEINLINE vector<T, 3> cross(const vector<T, 3> &lhs, const vector<T, 3> &rhs) {
...
template <class T>
_ALA_FORCEINLINE vector<T, 4> cross(const vector<T, 4> &lhs, const vector<T, 4> &rhs) {
	T m[] = {lhs[1] * rhs[2] - lhs[2] * rhs[1],
	         lhs[2] * rhs[0] - lhs[0] * rhs[2],
...
template <class T>
_ALA_FORCEINLINE vector<T, 8> cross(const vector<T, 8> &lhs, const vector<T, 8> &rhs) {
	T m[] = {lhs[1] * rhs[2] - lhs[2] * rhs[1],
	         lhs[2] * rhs[0] - lhs[0] * rhs[2],
