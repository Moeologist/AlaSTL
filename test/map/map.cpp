...
void test1() {
    ...
    // (2) 范围构造函数
    ala::map<std::string, int> iter(map1.find("anything"), map1.end());
    std::cout << "\niter = ";
    print_map(iter);
    std::cout << "map1 = ";
    print_map(map1);
    ...
    // (3) 复制构造函数
    ala::map<std::string, int> copied(map1);
    std::cout << "\ncopied = ";
    print_map(copied);
    std::cout << "map1 = ";
    print_map(map1);
    ...
    // (4) 移动构造函数
    ala::map<std::string, int> moved(std::move(map1));
    std::cout << "\nmoved = ";
    print_map(moved);
    std::cout << "map1 = ";
    print_map(map1);
    ...
    const ala::map<std::string, int> init{
        {"this", 100},
        {"can", 100},
        {"be", 100},
        {"const", 100},
    ...
    // 定制关键类选项 1 ：
    // 使用比较 struct
    ala::map<Point, double, PointCmp> mag = {{{5, -12}, 13},
                                             {{3, 4}, 5},
                                             {{-8, -15}, 17}};
...
void display_sizes(const ala::map<int, int> &nums1,
                   const ala::map<int, int> &nums2,
                   const ala::map<int, int> &nums3) {
    std::cout << "nums1: " << nums1.size() << " nums2: " << nums2.size()
              << " nums3: " << nums3.size() << '\n';
}

void test2() {
    ala::map<int, int> nums1{{3, 1}, {4, 1}, {5, 9}, {6, 1}, {7, 1}, {8, 9}};
    ala::map<int, int> nums2;
    ala::map<int, int> nums3;
...
void test3() {
    ala::map<char, int> letter_counts{{'a', 27}, {'b', 3}, {'c', 1}};
    ...
    ala::map<std::string, size_t> word_map;
...
void test5() {
    ...
    ala::map<Point *, double, PointPtrCmp> mag(
...
void test7() {
    ala::map<int, double> mag = {{1, 13}, {2, 5}, {3, 17}};
...
void test9() {
    ala::map<std::string, std::string> myMap;
...
void test10() {
    ala::map<std::string, std::string> m;
...
void test13() {
    ala::map<int, std::string> c = {{1, "one"},  {2, "two"},  {3, "three"},
                                    {4, "four"}, {5, "five"}, {6, "six"}};
    // 从 c 擦除所有奇数
...
void test14() {
    ala::map<int, std::string> m{{1, "mango"}, {2, "papaya"}, {3, "guava"}};
    auto nh = m.extract(2);
    nh.key() = 4;
...
void test15() {
    // 简单比较演示
    ala::map<int, char> example = {{1, 'a'}, {2, 'b'}};
    ...
    // 通透比较演示
    ala::map<FatKey, char, std::less<>> example2 = {{{1, {}}, 'a'},
                                                    {{2, {}}, 'b'}};
...
void test17() {
    const ala::map<int, const char *> m{
        {0, "zero"},
        {1, "one"},
        {2, "two"},
    };
